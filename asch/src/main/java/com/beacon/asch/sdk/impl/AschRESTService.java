...
import com.beacon.asch.sdk.AschInterface;
import com.beacon.asch.sdk.AschResult;
import com.beacon.asch.sdk.AschSDKConfig;
import com.beacon.asch.sdk.dbc.Argument;
import com.beacon.asch.sdk.security.SecurityStrategy;
import com.beacon.asch.sdk.transaction.TransactionBuilder;
...
public abstract class AschRESTService implements AschInterface {
    ...
    protected AschResult put(String relativeUrl, ParameterMap parameters) {
        try {
            String jsonString = REST.put(getFullUrl(relativeUrl), parameters, null, CHAR_SET);
            return AschResult.FromJsonString(jsonString);
        } catch (Exception ex) {
            return fail(ex);
        }
    ...
    protected AschResult put(String relativeUrl, String parameters) {
        try {
            String jsonString = REST.put(getFullUrl(relativeUrl), parameters, null, CHAR_SET);
            return AschResult.FromJsonString(jsonString);
        } catch (Exception ex) {
    ...
    protected AschResult putMagic(String relativeUrl, String parameters) {
        try {
            String jsonString = REST.put(getFullUrl(relativeUrl), parameters, getCustomeHeaders(), CHAR_SET);
            return AschResult.FromJsonString(jsonString);
        } catch (Exception ex) {
