...
public final class REST {
    ...
    protected static HttpResponse rawPut(String url, String requestBody, Map<String, String> customeHeads, String charset) throws IOException {
        CloseableHttpClient httpClient = HttpClients.createDefault();
        HttpPut httpPut = new HttpPut(encodeUrl(url));

        addCustomeHeads(customeHeads, httpPut);
        StringEntity entity = createEntity(requestBody, charset);
        httpPut.setEntity(entity);

        logger.debug(String.format("PUT url:%s, body:%s", url, requestBody));
    ...
    public static String put(String url, String parameters, Map<String, String> customeHeaders, String charset) throws IOException {
        try {
            HttpResponse response = rawPut(url, parameters, customeHeaders, charset);
